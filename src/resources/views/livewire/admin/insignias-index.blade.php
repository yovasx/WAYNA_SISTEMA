<div class="space-y-8">
    <x-admin.page-header eyebrow="Reconocimientos" title="Insignias" description="Administra las insignias que sostienen dinamicas de comunidad, gamificacion y reconocimiento.">
        <x-slot name="actions">
            <button type="button" wire:click="abrirModal" class="wayna-btn-primary">Nueva insignia</button>
        </x-slot>
    </x-admin.page-header>
    ...
    <x-admin.panel-card title="Catalogo de insignias" description="Mantiene el inventario de reconocimientos visibles para usuarios y campañas futuras.">
        <x-slot name="actions">
            <input wire:model.live.debounce.300ms="busqueda" type="text" placeholder="Buscar insignia" class="wayna-input lg:w-80">
        </x-slot>
        ...
        @if ($insignias->isEmpty())
            <x-admin.empty-state title="Sin insignias registradas" description="Crea la primera insignia para empezar a estructurar criterios de logros y reconocimientos." icon="workspace_premium">
                <x-slot name="action">
                    <button type="button" wire:click="abrirModal" class="wayna-btn-primary">Crear primera insignia</button>
                </x-slot>
            </x-admin.empty-state>
        @else
            <div class="wayna-table-wrap">
                <table class="wayna-table">
                    <thead class="wayna-table-head">
                        <tr>
                            <th class="wayna-table-th">Insignia</th>
                            <th class="wayna-table-th">Icono</th>
                            <th class="wayna-table-th">Criterio</th>
                        ...
                        @foreach ($insignias as $insignia)
                            <tr class="wayna-table-row">
                                <td class="wayna-table-td">
                                    <p class="font-medium text-ink">{{ $insignia->nombre }}</p>
                                    <p class="mt-1 text-sm text-ink-soft">{{ $insignia->descripcion ?: 'Sin descripcion registrada.' }}</p>
                                </td>
                                <td class="wayna-table-td break-all">{{ $insignia->icono ?: 'Sin icono' }}</td>
                                <td class="wayna-table-td">{{ $insignia->criterio_json ? 'Configurado' : 'Pendiente' }}</td>
                                <td class="wayna-table-td">
